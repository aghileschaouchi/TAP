...
(*** 2.1 ***)
  (*** L'inconveniant de cette technique est que dans le cas ou le premier element du couple ('a * 'astm) n'est pas evaluable une exception est leve dans tous les cas meme si on evalue le stm_tail de l'element  ***)
...
(* A revoir, version qui ne decongele pas pour recongeler 
  let rec stm_map_1 f fl = match fl with
    | StmEmpty -> StmEmpty
      | StmCons(x) -> StmCons(lazy(let u = (Lazy.force x) in f u), stm_map_1 f (stm_tail x));;
 *)
